...
class Price implements ProductHydratorInterface
{
    ...
    public function __construct(PriceCurrency $priceCurrency, CatalogHelper $catalogHelper, FloatComparator $floatComparator)
    {
    ...
    public function extract(Product $product): array
    {
        ...
        $priceWithTax = (float)$this->catalogHelper->getTaxPrice($product, $finalPrice, true);
        $oldPriceWithTax = (float)$this->catalogHelper->getTaxPrice($product, $price, true);

        $data = [];
        $data['price'] = $this->round($finalPrice);
        $data['old_price'] = $this->round($price);
        $data['price_with_tax'] = $this->round($priceWithTax);
        $data['old_price_with_tax'] = $this->round($oldPriceWithTax);
